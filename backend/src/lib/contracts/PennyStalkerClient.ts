...
import CONTRACT_ARC32 from './PennyStalker.arc32.json';

// ─── Types ───────────────────────────────────────────────────────────────────

/** Helper type for transactions with signers, often expected by ATC. */
...
export class PennyStalkerClient {
  ...
  async deploy(usdcAssetId: bigint): Promise<bigint> {
    ...
    // Helper to decode base64 without Buffer (for browser compatibility)
    const b64ToUint8 = (b64: string) => Uint8Array.from(atob(b64), (c) => c.charCodeAt(0));
    const b64ToUtf8 = (b64: string) => new TextDecoder().decode(b64ToUint8(b64));

    const approvalSource = b64ToUtf8(CONTRACT_ARC32.source.approval);
    const clearSource = b64ToUtf8(CONTRACT_ARC32.source.clear);
    ...
    const approvalProgram = b64ToUint8(approvalCompiled.result);
    const clearProgram = b64ToUint8(clearCompiled.result);
   ...
   */
  async createGoal(params: CreateGoalParams): Promise<{ goalId: bigint; txId: string }> {
    const sp = await this.getSuggestedParams();
    ...
    const result = await atc.execute(this.algod, 4);
    const goalId = BigInt(result.methodResults[0].returnValue as number);
    return { goalId, txId: result.txIDs[0] };
  }
  ...
  /**
   * Ensures the current sender is opted into the given asset.
   * If not opted in, it executes an opt-in transaction.
   *
   * @param assetId - The Asset ID to check
   */
  async ensureAssetOptIn(assetId: bigint): Promise<void> {
    try {
      // Check if already opted in
      await this.algod.accountAssetInformation(this.sender.addr, Number(assetId)).do();
    } catch (e: any) {
      // 404 means the account is not opted in
      if (e.status === 404) {
        console.log(`📡 Opting in ${this.sender.addr} to asset ${assetId}...`);
        const sp = await this.getSuggestedParams();
        const optInTxn = makeAssetTransferTxnWithSuggestedParamsFromObject({
          sender: this.sender.addr,
          receiver: this.sender.addr,
        ...
        const atc = new AtomicTransactionComposer();
        atc.addTransaction({ txn: optInTxn, signer: this.sender.signer });
        await atc.execute(this.algod, 4);
      } else {
        throw e;
      }
  ...
  async getNextGoalId(accountAddr: string): Promise<bigint> {
    ...
    const entry = locals.find(
      (kv) => new TextDecoder().decode(kv.key) === 'next_id',
    );
  ...
  async getUsdcAssetId(): Promise<bigint> {
    ...
    const entry = globals.find(
      (kv) => new TextDecoder().decode(kv.key) === 'usdc',
    );
    return entry ? BigInt(entry.value.uint) : 0n;
  }
}
